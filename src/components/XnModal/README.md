...
对 Ant Design `Modal` 的封装：默认居中、可拖拽标题栏、按系统配置限制整体高度（超出只滚 body）。

业务表单 / 确认弹窗请用 `XnDialog`（内部就是本组件）。不要直接用 antd `Modal`。命令式 `confirm` / `info` 等用本组件静态方法。
...
默认值读 `appConfig.ui.antd.modal`（`centered` / `draggable` / `maxHeight`）以及 `appConfig.ui.dialog.maxHeight`。命令式 API 只套用 `centered`，不可拖拽。关闭后位移归零。
